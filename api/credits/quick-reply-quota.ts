/**
 * Vercel Serverless Function - /api/credits/quick-reply-quota
 * 免费用户每日 2 次快捷回复配额校验与消耗
 */
...
export default async function handler(request: Request) {
  ...
  try {
    ...
    if (rpcError) {
      return jsonResponse(
        {
          error: 'Failed to consume quick reply quota',
      ...
      return jsonResponse(
        {
          ...
          feature: result.feature || 'quick_reply_generation',
          used: result.used,
          max: result.max
        },
        corsHeaders,
