...
first_purchase_events AS (
  SELECT DISTINCT ON (order_id)
    id, event_name, user_id, order_id, cta_source, occurred_at
  FROM business_events
  WHERE event_name = 'purchase_webhook_succeeded' AND order_id IS NOT NULL
...
normalized_events AS (
  SELECT id, event_name, user_id, order_id, cta_source, occurred_at
  FROM business_events
  WHERE event_name NOT IN ('purchase_webhook_succeeded', 'post_purchase_generation_success')
  UNION ALL SELECT * FROM first_purchase_events
  UNION ALL SELECT * FROM first_activation_events
)
SELECT
  ...
  COUNT(DISTINCT order_id) FILTER (WHERE event_name = 'purchase_webhook_succeeded') AS purchases,
  ...
  COUNT(DISTINCT order_id) FILTER (WHERE event_name = 'post_purchase_generation_success') AS post_purchase_generations,
  COUNT(DISTINCT user_id) FILTER (WHERE user_id IS NOT NULL) AS known_users,
  COUNT(*) FILTER (WHERE event_name = 'subscription_renewal_succeeded') AS subscription_renewals
FROM normalized_events
GROUP BY 1, 2;

ALTER VIEW public.conversion_funnel_daily SET (security_invoker = true);
REVOKE ALL ON public.conversion_funnel_daily FROM anon, authenticated, PUBLIC;
