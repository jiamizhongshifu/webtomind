-- Align Nano Banana 2 denoise credits with the billing alignment (2026-08-17):
-- nano-banana / nano-banana-2 share the same backend model
-- (gemini-3.1-flash-image-preview), so the 1.15 premium is removed:
-- (60 base + 20 * 2 references) * 1 = 100 credits.
-- Recreate the consume/refund RPCs with the new fixed cost. Already-charged
-- 115-credit transactions remain safe because refunds read the transaction
-- breakdown, not this constant.
