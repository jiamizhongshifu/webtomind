-- The cloud denoise pipeline performs two Nano Banana 2 generation calls. The
-- structure guide costs 92 credits (one reference) and the restoration costs
-- 115 credits (two references), following the shared image pricing policy:
-- (60 base + 20 per reference) * 1.15 model multiplier, rounded up per call.
-- Use a versioned RPC so the previous one-credit Worker remains safe during a
-- rolling deployment. Daily and referral credits remain excluded.

CREATE OR REPLACE FUNCTION public.consume_gpt_image_2_denoise_credits_v2(
  p_user_id UUID,
...
AS $consume_gpt_image_2_denoise_credits_v2$
DECLARE
  v_cost CONSTANT INTEGER := 207;
...
BEGIN
  ...
  PERFORM pg_advisory_xact_lock(
    hashtext('consume_gpt_image_2_denoise_credits_v2'),
...
END;
$consume_gpt_image_2_denoise_credits_v2$;

-- Keep one-credit refunds valid for tasks charged by the previous Worker. A
-- two-credit amount is also accepted for the short migration-only v2 window;
-- the production Worker charges 207. The wallet breakdown remains authoritative.
...
DECLARE
  ...
  v_refund_to TEXT;
BEGIN
  IF p_amount IS NULL OR p_amount NOT IN (1, 2, 207) THEN
  ...
  INSERT INTO public.credit_transactions (
    ...
    v_balance_after,
    p_source,
    'GPT Image 2 redraw failed; credits refunded',
...
COMMENT ON FUNCTION public.consume_gpt_image_2_denoise_credits_v2(UUID, JSONB)
IS 'Charges 207 credits for the two-stage GPT Image 2 cloud denoise pipeline: 92 for the structure guide and 115 for dual-reference restoration. Media wallets are preferred; subscription and bonus balances are safe fallbacks.';

COMMENT ON FUNCTION public.refund_gpt_image_2_denoise_credit(UUID, INTEGER, TEXT, TEXT, JSONB)
IS 'Idempotently restores a legacy one-credit, migration two-credit, or current 207-credit failed GPT Image 2 denoise charge to its original wallets.';

REVOKE ALL ON FUNCTION public.consume_gpt_image_2_denoise_credits_v2(UUID, JSONB)
FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.consume_gpt_image_2_denoise_credits_v2(UUID, JSONB)
TO service_role;
